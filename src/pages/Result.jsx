...
export default function Result() {
  ...
  const getBadge = (score) => {
    if (score >= 85) return { label: '🏆 EXCEPTIONAL', color: 'var(--success)' };
    if (score >= 70) return { label: '✅ STRONG', color: 'var(--text-secondary)' };
    if (score >= 50) return { label: '📈 AVERAGE', color: 'var(--warning)' };
    return { label: '💪 NEEDS WORK', color: 'var(--error)' };
  };

  const getStatusColor = (status) => {
    const map = { Strong: 'var(--success)', Good: 'var(--text-secondary)', Average: 'var(--warning)', Weak: 'var(--error)' };
    ...
    return (
      <div style={styles.loadingScreen}>
        <div style={styles.loadingCard}>
          ...
          <h2 style={styles.loadingTitle}>Analysing Performance</h2>
    ...
    <div style={styles.container}>
      ...
      <div style={styles.header}>
        ...
        <button style={styles.dashBtn}
          ...
          onMouseLeave={e => e.currentTarget.style.borderColor = 'rgba(197, 160, 89, 0.25)'}
        >
          DASHBOARD
        </button>
      </div>
        ...
        <div style={styles.scoreSection}>
          <div style={styles.scoreLeft}>
            ...
            <svg width="140" height="140" style={styles.scoreRing}>
              <circle cx="70" cy="70" r="54" fill="none" stroke="rgba(197, 160, 89, 0.08)" strokeWidth="6" />
              <circle cx="70" cy="70" r="54" fill="none" stroke="url(#purpleGrad)" strokeWidth="6"
          ...
          <div style={styles.tabs}>
            {answers.map((a, i) => (
              <button key={i}
                style={{
                  ...styles.tab,
                  background: activeTab === i ? 'rgba(197, 160, 89, 0.1)' : 'rgba(20, 16, 36, 0.3)',
                  borderColor: activeTab === i ? 'var(--primary)' : 'var(--border-color)',
                  color: activeTab === i ? 'var(--text-secondary)' : 'var(--text-muted)',
                }}
...
const styles = {
  ...
  loadingSub: { fontSize: '13px', color: 'var(--text-muted)', margin: 0 },
  loadingBar: {
    width: '240px',
    height: '4px',
  ...
  headerDivider: { width: '1px', height: '16px', background: 'var(--border-color)' },
  headerMeta: { fontSize: '11px', color: 'var(--text-secondary)', letterSpacing: '1px' },
  dashBtn: {
  ...
  dividerText: { fontSize: '8px', color: 'var(--text-muted)', letterSpacing: '3px', fontWeight: '700' },
  ...
  metaLabel: { fontSize: '8px', color: 'var(--text-muted)', letterSpacing: '2px', marginBottom: '4px', fontWeight: '700' },
  ...
  sectionRule: { flex: 1, height: '1px', background: 'var(--border-color)' },
  sectionTitle: {
    fontSize: '9px',
    color: 'var(--text-muted)',
    letterSpacing: '4px',
  ...
  navCount: { fontSize: '11px', color: 'var(--text-muted)', letterSpacing: '1px' },
